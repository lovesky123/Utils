...
public class FingerprintUtil {
    ...
    public static void callFingerPrint(final OnCallBackListener listener) {
        FingerprintManagerCompat managerCompat = FingerprintManagerCompat.from(MyApplication.getInstance());
        if (!managerCompat.isHardwareDetected()) { //判断设备是否支持
            if (listener != null)
                listener.onSupportFailed();
            return;
        }
        KeyguardManager keyguardManager = (KeyguardManager) MyApplication.getInstance().getSystemService(MyApplication.getInstance().KEYGUARD_SERVICE);
