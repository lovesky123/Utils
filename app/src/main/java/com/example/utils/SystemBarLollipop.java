...
@TargetApi(Build.VERSION_CODES.LOLLIPOP) class SystemBarLollipop {
    ...
    //view不根据系统窗口来调整自己的布局
        ...
        if(toolbar.getTag() == null) {
            ...
            toolbar.setPadding(toolbar.getPaddingLeft(), toolbar.getPaddingTop() + statusBarHeight, toolbar.getPaddingRight(),
            ...
            public void onOffsetChanged(AppBarLayout appBarLayout, int verticalOffset) {
                ...
                if(Math.abs(verticalOffset) > collapsingToolbarLayout.getScrimVisibleHeightTrigger()) {
                    if(appBarLayoutState != COLLAPSED) {
                        ...
                        if(SystemBarUtils.MIUISetStatusBarLightMode(activity, true)) {
                            return;
                        }
                        ...
                        } else if(!SystemBarUtils.FlymeSetStatusBarLightMode(activity, true)) {
                    ...
                    if(appBarLayoutState != EXPANDED) {
                        ...
                        if(SystemBarUtils.MIUISetStatusBarLightMode(activity, false)) {
                            translucentStatusBar(activity, true);
                            return;
                        }
                        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
